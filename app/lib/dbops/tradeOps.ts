...
const updateTrade = async (tradeId: string, trade: Trade): Promise<Trade> => {
  if (!trade || !trade.id || !tradeId) throw new Error('Trade data with valid ID is required ,  and tradeId is required.');
